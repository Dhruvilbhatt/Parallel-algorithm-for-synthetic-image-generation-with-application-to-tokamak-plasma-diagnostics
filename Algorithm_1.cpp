...
void function_interim(double tt, double x1, double y1, double z1, double x2, double y2, double z2, double r, double a_e, double &pixel_intensity, double t,double pro_type,double T_zero)
{
					int end = 1/t;
		  			...
		  			#pragma omp parallel for reduction(+:temp_intensity)  
...
int main()
{
    ...
    fp_static = fopen("time_naive.txt", "a+");
    ...
    double *x_sp = new double [int(pixels) * 8], *y_sp = new double [int(pixels) * 8], *z_sp = new double [int(pixels) * 8], *intensity_value = new double [int(pixels)];
    ...
    if(pinhole_x > 0)
    {
        ...
        for (int i = 0; i < (int)dimn ; i++)
        {
            ...
            {
                    x_sp[int(i*dimn+j) * 8] = x;
                    y_sp[int(i*dimn+j) * 8] = y - j*side;
                    z_sp[int(i*dimn+j) * 8] = z + i*side; 
            }
        }
    }
    ...
    for (int i = 0; i < (int)pixels; i++)
    {
                ...
                double x_i = x_sp[i * 8], y_i = y_sp[i * 8], z_i = z_sp[i * 8];
    ...
    cout<<"Total Parallel Time for Algorithm I: "<<final_total_time<<" Pixels : "<<(int)sqrt(pixels)<<"\n";
}
